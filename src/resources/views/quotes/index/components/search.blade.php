{!! Form::open(['url' => route('killerquotes.index'), 'method' => 'get', 'id' => 'formFilterKillerQuotes']) !!}
<div class="row @if(!request()->input()) d-none @endif" id="advancedSearchBox">

    @if(request()->input())
        <div style="float: left;width:87px;">
            <div class="form-group">
                <a href="{{route('killerquotes.index')}}" class="btn btn-success" id="refresh" title="reset ricerca"><i class="fa fa-redo"></i> Reset</a>
            </div>
        </div>
    @endif
    ...
    <div class="col-12 col-sm-4 col-lg-2">
        <div class="form-group">
            <select id="expired" name="expired">
                <option @if(!request()->has('expired')) selected @endif></option>
                <option value="1" @if(request('expired') == '1') selected @endif >Sì</option>
                <option value="0" @if(request('expired') == '0') selected @endif >No</option>
            </select>
        </div>
    </div>
...
@push('scripts')
    <script>
        ...
        function submitter()
        {
            ...
            go += ($('select[name="expired"]').find('option:selected').val() == '') ? 0 : 1;
            ...
            if(go > 0)
            {
                $('#formFilterKillerQuotes').submit();
            }
            else
            {
                window.location.href = baseURL + 'killerquotes';
            }

        }

        $('select#expired').select2({theme: 'bootstrap4', width: '100%', placeholder: 'Scaduto', allowClear: true});
        ...
        $('select[name="expired"]').on('change', function(){
            submitter();
        });
